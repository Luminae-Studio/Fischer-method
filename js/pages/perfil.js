...
function _perfilRenderHeader() {
  ...
  hdr.innerHTML =
    '<div style="text-align:center;padding:28px 0 22px;">' +
      ...
      '</div>' +
      (p.email
        ? '<div style="font-size:12px;color:var(--muted);margin-top:3px;">' + p.email + '</div>'
        : '') +
      (p.bio
        ? '<div style="font-size:12px;color:var(--muted);margin-top:10px;line-height:1.6;max-width:280px;margin-left:auto;margin-right:auto;">' + p.bio + '</div>'
        : '') +
      (p.telefone
        ? '<div style="font-size:12px;color:var(--muted);margin-top:4px;">&#x1F4DE; ' + p.telefone + '</div>'
        : '') +
...
function abrirEdicaoPerfil() {
  ...
  hdr.innerHTML =
    '<div style="padding:20px 0 8px;">' +
      ...
      '<div class="fg">' +
        '<label>Foto (URL de imagem)</label>' +
        '<input type="url" id="ep-foto" value="' + _escHtml(p.photo_url || '') + '" placeholder="https://...">' +
      '</div>' +

      '<div class="fg">' +
        '<label>Nome completo</label>' +
        '<input type="text" id="ep-nome" value="' + _escHtml(p.name || '') + '">' +
      '</div>' +

      '<div class="fg">' +
        '<label>Telefone</label>' +
        '<input type="tel" id="ep-tel" value="' + _escHtml(p.telefone || '') + '" placeholder="(11) 99999-9999">' +
      '</div>' +

      '<div class="fg">' +
        '<label>Bio / Apresentacao</label>' +
        '<textarea id="ep-bio" rows="3" placeholder="Personal trainer especializado em...">' +
          _escHtml(p.bio || '') +
        '</textarea>' +
      '</div>' +
  ...
  setTimeout(function() {
    var inp = document.getElementById('ep-nome');
    if (inp) inp.focus();
  }, 100);
}
...
async function salvarPerfil() {
  ...
  try {
    var data = {
      name:      nome,
      photo_url: (document.getElementById('ep-foto').value || '').trim() || null,
    ...
    if (err) {
      toast('Erro ao salvar: ' + (err.message || JSON.stringify(err)));
      return;
    }
...
function _perfilConfigHTML() {
  ...
  return (
    '<div style="font-size:11px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:.08em;margin-bottom:14px;">Configuracoes</div>' +

    // ── Tema ────────────────────────────────────
    '<div style="font-size:12px;color:var(--muted);margin-bottom:8px;font-weight:600;">Tema</div>' +
    '<div style="display:flex;gap:8px;margin-bottom:22px;">' +
      _temaBtn('escuro', '&#x1F319; Escuro', temaAtual === 'escuro') +
      _temaBtn('claro',  '&#x2600; Claro',   temaAtual === 'claro')  +
    '</div>' +

    // ── Notificações ────────────────────────────
    '<div style="font-size:12px;color:var(--muted);margin-bottom:4px;font-weight:600;">Notificacoes</div>' +
    '<div class="card" style="padding:0 14px;margin-bottom:28px;">' +
      _perfilToggle(
        'toggle-nf-feedback',
        'Feedback de aluno',
        'Aviso quando um aluno registra feedback',
        nfFeedback,
        'perfilToggleNotif(\'fm_notif_feedback\',\'toggle-nf-feedback\')',
        true
      ) +
      _perfilToggle(
        'toggle-nf-treino',
        'Treino concluido',
        'Aviso quando um aluno conclui um treino',
        nfTreino,
        'perfilToggleNotif(\'fm_notif_treino\',\'toggle-nf-treino\')',
        true
      ) +
    '</div>'
  );
...
function perfilSetTema(tema) {
  ...
  // Atualiza visual dos botões sem re-render completo
...
function perfilToggleNotif(key, btnId) {
  var atual = localStorage.getItem(key) === '1';
  var novo  = !atual;
...
function perfilLogout() {
  if (confirm('Sair da conta Fischer Method?')) {
    logout();
  }
}
